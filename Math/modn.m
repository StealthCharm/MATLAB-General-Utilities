%[text] Simple wrapper function to inline the replacement of zero values with that of the divisor.
function A = modn(A, m)
    arguments
        A {mustBeMathematic}
        m {mustBeMathematic}
    ...
    % Perform modulo operation and replace the zero values with m
    A = mod(A - 1, m) + 1;
end
